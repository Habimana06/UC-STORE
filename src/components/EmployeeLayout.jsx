...
import {
  LogOut, Home, Package, ShoppingCart, Settings as SettingsIcon,
  User, BarChart3, Menu, X, Bell, Search, Clock, ChevronLeft, ChevronRight,
  Activity, TrendingUp, AlertTriangle, CheckCircle, Calendar, 
  DollarSign, PieChart, Store
} from "lucide-react";
...
export default function EmployeeLayout() {
  ...
  const [notifications] = useState([
    { id: 1, type: 'warning', message: 'Low stock: USB-C Cable', time: '5m ago' },
    { id: 2, type: 'success', message: 'Order #3012 completed', time: '18m ago' },
    { id: 3, type: 'info', message: 'Shift schedule updated', time: '1h ago' }
  ]);
  ...
  const quickActions = [
    { icon: TrendingUp, label: "Quick Stats", color: "text-green-400", to: "/employee" },
    { icon: DollarSign, label: "Sales", color: "text-blue-400", to: "/employee/sales" },
    { icon: Package, label: "Products", color: "text-purple-400", to: "/employee/products" },
    { icon: Calendar, label: "Analytics", color: "text-orange-400", to: "/employee/analytics" }
  ];
    ...
    <div className={`min-h-screen transition-colors duration-300 ${
      ...
      `}>
        ...
        <div className="p-4 border-b border-gray-700/50 flex-shrink-0">
              ...
              <div className="flex items-center gap-2">
                ...
                {!sidebarCollapsed && (
                  <div>
                    ...
                    <div className="text-xs text-gray-400">Employee Panel</div>
                  </div>
                )}
              </div>
            </div>
          ...
          {!sidebarCollapsed && (
            <div className="mt-3">
              <span className="text-xs px-3 py-1.5 rounded-full text-white font-semibold bg-gradient-to-r from-blue-500 to-cyan-500 flex items-center gap-1 w-fit">
                    ...
                    {!sidebarCollapsed && (
                      <>
                        <span className="flex-1">{
                          (() => {
                            switch(item.path){
                              case '/employee': return t('nav.home');
                              case '/employee/products': return t('nav.products');
                              case '/employee/sales': return t('nav.sales');
                              case '/employee/analytics': return t('nav.analytics');
                              case '/employee/profile': return t('nav.profile');
        ...
        <header className={`backdrop-blur-xl border-b shadow-sm flex-shrink-0 sticky top-0 z-20 transition-colors duration-300 ${
          ...
          <div className="px-6 py-4 flex items-center justify-between transition-all duration-300">
            ...
            <div className="flex items-center gap-3 ml-auto">
                ...
                {showNotifications && (
                  <div className={`absolute right-0 top-full mt-2 w-80 rounded-xl shadow-xl border z-50 max-h-96 overflow-y-auto ${
                    ...
                    <div className="p-4 border-b border-gray-100 dark:border-gray-800">
                      <div className="flex items-center justify-between">
                        <h3 className="font-semibold text-gray-900 dark:text-gray-100">{t('actions.notifications')}</h3>
                        <span className="text-xs text-gray-500 dark:text-gray-400">{notifications.length} {t('actions.new')}</span>
                      </div>
                    </div>
                    <div className="p-2">
              ...
              {/* Current path */}
              <div className="hidden sm:flex items-center gap-2 px-3 py-2 bg-gray-50 dark:bg-gray-800 rounded-xl">
              ...
              <div className="hidden sm:flex items-center gap-2">
                <button 
                  onClick={toggleTheme} 
                  className="px-3 py-2 rounded-lg text-xs bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors border border-gray-200 dark:border-gray-700"
                >
                ...
                <select
                  ...
                  className="px-2 py-2 rounded-lg text-xs bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200"
                >
              ...
              {/* User menu */}
              <div className="flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 rounded-xl border border-gray-200 dark:border-gray-700">
                <User size={16} className="text-gray-500 dark:text-gray-300" />
                <span className="font-medium text-gray-700 dark:text-gray-200 text-sm">{user?.username}</span>
              </div>
